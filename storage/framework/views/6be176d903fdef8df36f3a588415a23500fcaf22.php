...
;(function($, window, document) {
  ...
  {
      var link = '<?php echo e(route("admin.support.chat_conversation.show", ":slug"), false); ?>';
      ...
      return $('<div>').attr('id', 'chat-'+msgObj.customer_id).addClass('row sidebarBody').append(
            $('<a>').attr('href', 'javascript:void(0)').attr('data-link', link).addClass('get-content').append(
              $('<div>').addClass('col-sm-3 col-xs-3').append(
                  $('<img/>').attr('src', msgObj.avatar).attr('alt', '<?php echo e(trans('app.avatar'), false); ?>').addClass('img-circle')
              )
            ).append(
  ...
  function prepareNewChatMsg(txt, who = 'sender')
  {
      return $('<div>').addClass('row message-body').append(
            $('<div>').addClass('col-sm-12 message-main-'+who).append(
              ...
              $('<span>').addClass('message-time').text('<?php echo e(trans('theme.now'), false); ?>')
            )
      );
  }
  ...
  $( document ).ready(function () {
            ...
            complete: function (xhr, textStatus) {
              switch (xhr.status) {
                ...
                case 401:
                    ...
                    response = $('<p>').addClass('text-danger').text("<?php echo trans('messages.session_expired'); ?>");
                    $('<br/><br/>').prependTo(response);
                    $('<a>').attr('href', "javascript:void(0)").attr('data-toggle', "modal").attr('data-target', "#loginModal").addClass('btn btn-primary').text("<?php echo e(trans('app.login'), false); ?>").appendTo(response);
                    break;
                case 403:
                case 419:
                    $("#conversationBox").html(""); //Clear the chatbox
                    response = $('<p>').addClass('text-danger').text("<?php echo trans('messages.session_expired'); ?>");
                    $('<br/><br/>').prependTo(response);
                    $('<a>').attr('href', "<?php echo e(route('customer.login'), false); ?>").addClass('btn btn-primary').text("<?php echo e(trans('app.login'), false); ?>").appendTo(response);
                    break;
                default:
                    response = $('<div>').addClass('row message-body').append(
                          $('<div>').addClass('col-sm-12').append(
                            $('<p class="lead">').addClass('text-danger').text("<?php echo trans('messages.failed'); ?>")
                          )
                    );
                    $('<br/><br/>').prependTo(response);
              }
    ...
    var channel = pusher.subscribe('<?php echo e(get_vendor_chat_room_id(), false); ?>');
...
</style><?php /**PATH /home/amraibes/public_html/resources/views/plugins/chatbox.blade.php ENDPATH**/ ?>