    ...
    */
    ...
    public function login(Request $request)
    {
      ...
      if ($this->attemptLogin($request)) {
        ...
        return response()->json(['email'=>'Incorrent username or password.'],404);
      }
    ...
    protected function attemptLogin(Request $request)
    {
       $fieldType = filter_var($request->email, FILTER_VALIDATE_EMAIL) ? 'email' : 'mobile';
        return Auth::guard('customer')->attempt([$fieldType=>$request->email, 'password'=>$request->password], $request->filled('remember')
        );
    }
